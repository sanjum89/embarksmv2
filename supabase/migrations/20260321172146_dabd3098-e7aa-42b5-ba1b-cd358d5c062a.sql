...
CREATE TABLE public.nudge_cards (
...
-- Enable RLS
ALTER TABLE public.nudge_cards ENABLE ROW LEVEL SECURITY;

-- Public read/write for demo (no auth)
CREATE POLICY "Anyone can read nudge_cards"
  ON public.nudge_cards FOR SELECT USING (true);

CREATE POLICY "Anyone can insert nudge_cards"
  ON public.nudge_cards FOR INSERT WITH CHECK (true);

CREATE POLICY "Anyone can update nudge_cards"
  ON public.nudge_cards FOR UPDATE USING (true);

CREATE POLICY "Anyone can delete nudge_cards"
  ON public.nudge_cards FOR DELETE USING (true);
