...
CREATE TABLE embarksmv2.competency_catalog (
...
CREATE TABLE embarksmv2.role_competency_requirements (
  id uuid NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  account_id uuid NOT NULL,
...
CREATE TABLE embarksmv2.persona_competency_profiles (
  id uuid NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  account_id uuid NOT NULL,
...
CREATE TABLE embarksmv2.module_competency_tags (
  id uuid NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  account_id uuid NOT NULL,
...
CREATE TABLE embarksmv2.persona_module_adaptations (
  id uuid NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  account_id uuid NOT NULL,
  persona_code text NOT NULL,
  module_code text NOT NULL,
...
ALTER TABLE embarksmv2.competency_catalog ENABLE ROW LEVEL SECURITY;
ALTER TABLE embarksmv2.role_competency_requirements ENABLE ROW LEVEL SECURITY;
ALTER TABLE embarksmv2.persona_competency_profiles ENABLE ROW LEVEL SECURITY;
ALTER TABLE embarksmv2.module_competency_tags ENABLE ROW LEVEL SECURITY;
ALTER TABLE embarksmv2.persona_module_adaptations ENABLE ROW LEVEL SECURITY;
...
BEGIN
  ...
  LOOP
    EXECUTE format('CREATE POLICY "anon_select_%1$s" ON embarksmv2.%1$s FOR SELECT USING (true);', t);
    ...
    EXECUTE format('CREATE TRIGGER set_updated_at_%1$s BEFORE UPDATE ON embarksmv2.%1$s FOR EACH ROW EXECUTE FUNCTION embarksmv2.set_updated_at();', t);
  END LOOP;
END$$;

CREATE INDEX idx_pcp_account_persona ON embarksmv2.persona_competency_profiles (account_id, persona_code);
CREATE INDEX idx_mct_account_module ON embarksmv2.module_competency_tags (account_id, module_code);
CREATE INDEX idx_pma_account_persona ON embarksmv2.persona_module_adaptations (account_id, persona_code);
CREATE INDEX idx_rcr_account_role ON embarksmv2.role_competency_requirements (account_id, role_cohort_code);
