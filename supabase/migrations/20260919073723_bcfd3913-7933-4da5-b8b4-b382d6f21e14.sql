...
CREATE OR REPLACE FUNCTION public.mirror_id(p_target uuid, p_id uuid)
RETURNS uuid
LANGUAGE sql
IMMUTABLE
...
CREATE OR REPLACE FUNCTION public.mirror_account_content(p_source uuid, p_target uuid)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
BEGIN
  ...
  FOR i IN REVERSE array_length(v_tables, 1)..1 LOOP
    EXECUTE format('DELETE FROM public.%I WHERE account_id = $1', v_tables[i]) USING p_target;
  END LOOP;

  -- Copy rows in dependency order
  FOREACH t IN ARRAY v_tables LOOP
    ov := 'jsonb_build_object(''account_id'', $2, ''id'', public.mirror_id($2, x.id))';

    IF v_fks ? t THEN
      FOR col IN SELECT jsonb_array_elements_text(v_fks -> t) LOOP
        ov := ov || format(' || jsonb_build_object(%L, public.mirror_id($2, x.%I))', col, col);
      END LOOP;
    END IF;
    ...
    EXECUTE format(
      'INSERT INTO public.%1$I SELECT (jsonb_populate_record(NULL::public.%1$I, to_jsonb(x) || %2$s)).* FROM public.%1$I x WHERE x.account_id = $1',
      t, ov
    ...
    IF v_selfrefs ? t THEN
      FOR col IN SELECT jsonb_array_elements_text(v_selfrefs -> t) LOOP
        EXECUTE format(
          'UPDATE public.%1$I d SET %2$I = public.mirror_id($2, s.%2$I) FROM public.%1$I s WHERE s.account_id = $1 AND d.id = public.mirror_id($2, s.id) AND s.%2$I IS NOT NULL',
          t, col
        ) USING p_source, p_target;
      END LOOP;
    END IF;
...
REVOKE ALL ON FUNCTION public.mirror_account_content(uuid, uuid) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.mirror_account_content(uuid, uuid) TO service_role;
GRANT EXECUTE ON FUNCTION public.mirror_id(uuid, uuid) TO authenticated, service_role;
