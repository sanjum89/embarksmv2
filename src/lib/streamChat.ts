...
const CHAT_URL = `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/embarksmv2-chat`;
const ROLE_PLAY_CHAT_URL = `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/embarksmv2-role-play-chat`;
...
export async function streamRolePlayChat({
  ...
  try {
    const resp = await fetch(ROLE_PLAY_CHAT_URL, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${import.meta.env.VITE_SUPABASE_PUBLISHABLE_KEY}`,
    ...
    console.error("[RolePlay] Unexpected error:", err);
    onError?.(err instanceof Error ? err.message : "Unexpected error connecting to AI");
    onDone();
  }
}
