...
-- ENUMS
CREATE TYPE public.difficulty_level AS ENUM ('foundation','practitioner','advanced','leadership');
CREATE TYPE public.delivery_mode AS ENUM ('digital','offline','blended','live_cohort','simulation','workplace_practice','coaching');
CREATE TYPE public.chapter_content_type AS ENUM ('reading','video','case_study','client_scenario','simulation','system_practice','shadowing_task','coaching_discussion','mentor_discussion','peer_discussion','reflection','quiz','observed_practice','workplace_assignment');
CREATE TYPE public.assessment_type AS ENUM ('diagnostic_knowledge_check','applied_client_scenario','investment_research_case','portfolio_construction_case','suitability_review','consumer_duty_judgement_case','regulatory_knowledge_check','ethical_dilemma','client_conversation_simulation','market_volatility_scenario','business_development_plan','observed_client_meeting','mentor_review','manager_signoff','assessor_review','readiness_board_evidence_pack','cpd_reflection','system_workflow_completion','ai_tool_usage_reflection');
CREATE TYPE public.assessment_scope AS ENUM ('milestone','module_post','readiness_gate','adhoc');
CREATE TYPE public.evidence_type AS ENUM ('quiz_score','written_case_response','simulation_score','observed_behaviour_note','manager_signoff','mentor_feedback','assessor_review','cpd_reflection','system_task_completion','workplace_assignment','client_meeting_observation','readiness_board_pack');
CREATE TYPE public.reviewer_role AS ENUM ('manager','mentor','assessor','peer','self');
CREATE TYPE public.submission_format AS ENUM ('written','upload','observation','system_record','recording');
CREATE TYPE public.readiness_outcome AS ENUM ('not_ready','ready_with_support','ready','ready_for_stretch');
CREATE TYPE public.agent_action AS ENUM ('continue','micro_learn','reopen_and_lock','manager_review');
CREATE TYPE public.gate_requirement_kind AS ENUM ('track','module','assessment','evidence');
CREATE TYPE public.promotion_trigger AS ENUM ('readiness_gate','stretch_completion','manager_initiated');
...
CREATE TABLE public.catalog_modules (
  ...
  recommended_delivery_mode public.delivery_mode NOT NULL DEFAULT 'digital',
...
CREATE INDEX idx_catalog_modules_role ON public.catalog_modules (account_id, role_cohort_code, learning_track_code);
...
CREATE TABLE public.catalog_chapters (
  ...
  content_type public.chapter_content_type NOT NULL DEFAULT 'reading',
  estimated_time_minutes INT NOT NULL DEFAULT 25,
  difficulty_level public.difficulty_level NOT NULL DEFAULT 'foundation',
  delivery_mode public.delivery_mode NOT NULL DEFAULT 'digital',
...
CREATE INDEX idx_catalog_chapters_module ON public.catalog_chapters (account_id, module_code, display_order);

CREATE TABLE public.catalog_assessment_blueprints (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(), account_id UUID NOT NULL,
  blueprint_code TEXT NOT NULL, module_code TEXT NOT NULL, chapter_code TEXT,
  scope public.assessment_scope NOT NULL DEFAULT 'module_post',
...
CREATE INDEX idx_blueprints_module ON public.catalog_assessment_blueprints (account_id, module_code);

CREATE TABLE public.catalog_evidence_tasks (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(), account_id UUID NOT NULL,
  ...
  reviewer_role public.reviewer_role NOT NULL DEFAULT 'manager',
  submission_format public.submission_format NOT NULL DEFAULT 'written',
...
CREATE TABLE public.catalog_readiness_gates (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(), account_id UUID NOT NULL,
...
CREATE TABLE public.catalog_gate_requirements (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(), account_id UUID NOT NULL,
  gate_code TEXT NOT NULL,
  requirement_kind public.gate_requirement_kind NOT NULL,
...
CREATE TABLE public.employee_persona_assignments (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(), account_id UUID NOT NULL,
...
CREATE UNIQUE INDEX uq_learner_progress_chapter ON public.learner_progress (cohort_id, employee_id, chapter_code) WHERE chapter_code IS NOT NULL;
CREATE UNIQUE INDEX uq_learner_progress_module ON public.learner_progress (cohort_id, employee_id, module_code) WHERE chapter_code IS NULL;
CREATE INDEX idx_learner_progress_lookup ON public.learner_progress (account_id, employee_id, cohort_id);

CREATE TABLE public.chapter_lock_events (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(), account_id UUID NOT NULL,
  cohort_id UUID NOT NULL REFERENCES public.cohorts(id) ON DELETE CASCADE,
  employee_id TEXT NOT NULL,
...
CREATE TABLE public.assessment_instances (
  ...
  kind public.assessment_scope NOT NULL,
  ...
  agent_action public.agent_action,
...
CREATE INDEX idx_assessment_instances_lookup ON public.assessment_instances (account_id, employee_id, cohort_id);

CREATE TABLE public.micro_learnings (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(), account_id UUID NOT NULL,
  cohort_id UUID NOT NULL REFERENCES public.cohorts(id) ON DELETE CASCADE,
  employee_id TEXT NOT NULL,
  source_assessment_id UUID NOT NULL REFERENCES public.assessment_instances(id) ON DELETE CASCADE,
...
CREATE TABLE public.readiness_gate_results (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(), account_id UUID NOT NULL,
  cohort_id UUID NOT NULL REFERENCES public.cohorts(id) ON DELETE CASCADE,
  employee_id TEXT NOT NULL, gate_code TEXT NOT NULL,
  outcome public.readiness_outcome,
  ai_recommendation public.readiness_outcome,
  ai_rationale TEXT,
  manager_decision public.readiness_outcome,
  manager_notes TEXT, manager_signed_off_at TIMESTAMPTZ,
  assessor_decision public.readiness_outcome,
...
CREATE TABLE public.promotion_signals (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(), account_id UUID NOT NULL,
  ...
  triggered_by public.promotion_trigger NOT NULL DEFAULT 'readiness_gate',
